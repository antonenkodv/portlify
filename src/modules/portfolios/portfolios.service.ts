...
export class PortfoliosService {
    ...
    async findAll(user :Pick<User , 'id'>){
        return this.portfolioRepository.findAll({where : {userId : user.id}})
    }
    async findOne(user :Pick<User , 'id'>,portfolioId: string){
        return this.portfolioRepository.findOne({where : {userId : user.id , id: portfolioId}})
    }
    async destroy(user :Pick<User , 'id'>,portfolioId: string){
